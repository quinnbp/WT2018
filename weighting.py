...
def voting(weighting_input):
    """Creates label:weight dictionary, voting_list for each classifier
    ...
    for pair in weighting_input:
        ...
        weight_dict = dict()
        label = 0
        for weight in pair[0].get_precision():  # precision = [0.7, 0.2, 0.1]
    ...
    weighting_options = ["Precision", "CEN", "Equal Vote"]
...
def multi_cls_vote(n_results, v_list, w_type):
    ...
    if w_type == "Precision":
        ...
        return CEN_vote(n_results, v_list)
    elif w_type == "CEN_Precision":
        return CEN_precision_vote(n_results, v_list)
...
def precision_vote(n_results, v_list):
    ...
    for i in range(n_results):
        ...
        for cls_votes in v_list[cls_num]:
            ...
            #print("List of cls votes:", cls_votes)
            #print("Sample vote", cls_votes[1])
...
def CEN_vote(n_results, v_list): # CEN score
    return

def CEN_precision_vote(n_results, v_list): # (1 - CEN)* Precision !!!!
    return
